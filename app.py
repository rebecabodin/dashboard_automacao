...
if not df_captacao.empty:
    ...
    if menu_selecionado == '📊 Visão Principal':
        ...
        try:
            ...
            # --- DRE & ROI DO INVESTIMENTO EM DISPAROS DE WHATSAPP ---
            st.markdown("""
            <div style="background-color:#0f172a; border-left:6px solid #10b981; padding:20px 24px; border-radius:12px; margin-bottom:16px; box-shadow:0 4px 20px rgba(0,0,0,0.35); color:#ffffff;">
                <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
                    <div>
                        ...
                        <h3 style="color:#ffffff; font-weight:800; margin:8px 0 0 0; font-size:1.4rem;">💵 Demonstrativo Financeiro & ROI do WhatsApp</h3>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)

            tab_dre_sem, tab_dre_acum = st.tabs(["📅 1. Semana de Fechamento (17 a 24/08)", "📆 2. Acumulado do Lançamento (27/07 a 24/08)"])

            # ---------------------------------------------------------
            # TAB 1: SEMANA DE FECHAMENTO (17 A 24/08)
            # ---------------------------------------------------------
            with tab_dre_sem:
                custo_sem_usd = 49.11
                custo_sem_brl = custo_sem_usd * 5.60
                lucro_sem_brl = roi_resgatado_wpp - custo_sem_brl
                roi_mult_sem = roi_resgatado_wpp / custo_sem_brl if custo_sem_brl > 0 else 0

                val_sem_brl = f"{custo_sem_brl:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
                val_sem_resg = f"{roi_resgatado_wpp:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
                val_sem_lucro = f"{lucro_sem_brl:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
                val_sem_roi_pct = f"{roi_mult_sem*100:,.0f}".replace(',', 'X').replace('.', ',').replace('X', '.')

                st.markdown(f"""
                <div style="background-color:#064e3b; border:2px solid #10b981; padding:12px 18px; border-radius:10px; margin-bottom:16px; display:flex; justify-content:space-between; align-items:center; color:#ffffff;">
                    <span style="font-weight:700; font-size:1rem; color:#ffffff;">📊 Resultado Financeiro da Semana de Fechamento (17 a 24/08)</span>
                    <span style="background-color:#10b981; color:#ffffff; font-weight:800; padding:4px 12px; border-radius:8px; font-size:1.1rem;">⚡ {roi_mult_sem:.1f}x Retorno (ROI {val_sem_roi_pct}%)</span>
                </div>
                """, unsafe_allow_html=True)
                ...
                with c_s1:
                    ...
                    <div style="background-color:#0f172a; border-top:4px solid #3b82f6; padding:18px; border-radius:10px; text-align:center; color:#ffffff; min-height:140px;">
                        <span style="font-size:0.78rem; color:#bfdbfe; text-transform:uppercase; font-weight:700;">💳 Custo de Disparos (Semana)</span>
                        <h3 style="color:#ffffff; font-weight:800; margin:8px 0; font-size:1.4rem;">US$ {custo_sem_usd:.2f} <span style="font-size:0.85rem; color:#94a3b8;">(~R$ {val_sem_brl})</span></h3>
                ...
                with c_s2:
                    ...
                    <div style="background-color:#064e3b; border-top:4px solid #10b981; padding:18px; border-radius:10px; text-align:center; color:#ffffff; min-height:140px;">
                        ...
                        <h3 style="color:#ffffff; font-weight:800; margin:8px 0; font-size:1.4rem;">R$ {val_sem_resg}</h3>
                ...
                with c_s3:
                    ...
                    <div style="background-color:#065f46; border-top:4px solid #34d399; padding:18px; border-radius:10px; text-align:center; color:#ffffff; min-height:140px;">
                        <span style="font-size:0.78rem; color:#a7f3d0; text-transform:uppercase; font-weight:700;">💰 Lucro Líquido da Semana</span>
                        <h3 style="color:#ffffff; font-weight:800; margin:8px 0; font-size:1.4rem;">R$ {val_sem_lucro}</h3>
                        <span style="font-size:0.75rem; color:#34d399;">Faturamento - Custo Semanal</span>
                    </div>
                    """, unsafe_allow_html=True)

            # ---------------------------------------------------------
            # TAB 2: ACUMULADO COMPLETO DO LANÇAMENTO LC7 (27/07 A 24/08)
            # ---------------------------------------------------------
            with tab_dre_acum:
                custo_acum_usd = 260.86
                custo_acum_brl = custo_acum_usd * 5.60 # Câmbio estimado R$ 5,60/USD (~R$ 1.460,82)
                lucro_acum_resg = roi_resgatado_wpp - custo_acum_brl
                roi_mult_resg = roi_resgatado_wpp / custo_acum_brl if custo_acum_brl > 0 else 0
                roi_mult_glob = faturamento_global / custo_acum_brl if custo_acum_brl > 0 else 0

                val_acum_brl = f"{custo_acum_brl:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
                val_acum_resg = f"{roi_resgatado_wpp:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
                val_acum_lucro = f"{lucro_acum_resg:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
                val_acum_roi_resg = f"{roi_mult_resg*100:,.0f}".replace(',', 'X').replace('.', ',').replace('X', '.')
                val_acum_roi_glob = f"{roi_mult_glob*100:,.0f}".replace(',', 'X').replace('.', ',').replace('X', '.')

                st.markdown(f"""
                <div style="background-color:#0f172a; border:2px solid #3b82f6; padding:12px 18px; border-radius:10px; margin-bottom:16px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px; color:#ffffff;">
                    <span style="font-weight:700; font-size:0.95rem; color:#ffffff;">📆 Resultado Consolidado do Mês (27/07 a 24/08)</span>
                    <div>
                        <span style="background-color:#10b981; color:#ffffff; font-weight:800; padding:4px 10px; border-radius:8px; font-size:0.95rem; margin-right:6px;">🚀 ROI Resgatado WPP: {roi_mult_resg:.1f}x ({val_acum_roi_resg}%)</span>
                        <span style="background-color:#3b82f6; color:#ffffff; font-weight:800; padding:4px 10px; border-radius:8px; font-size:0.95rem;">🏆 ROI Global Checkout: {roi_mult_glob:.1f}x ({val_acum_roi_glob}%)</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                with c_a1:
                    st.markdown(f"""
                    <div style="background-color:#0f172a; border-top:4px solid #3b82f6; padding:18px; border-radius:10px; text-align:center; color:#ffffff; min-height:140px;">
                        <span style="font-size:0.78rem; color:#bfdbfe; text-transform:uppercase; font-weight:700;">💳 Custo Total Mês API</span>
                        <h3 style="color:#ffffff; font-weight:800; margin:8px 0; font-size:1.4rem;">US$ {custo_acum_usd:.2f} <span style="font-size:0.85rem; color:#94a3b8;">(~R$ {val_acum_brl})</span></h3>
                        <span style="font-size:0.75rem; color:#60a5fa;">15.687 Utility + 1.929 Marketing Msgs</span>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                with c_a2:
                    st.markdown(f"""
                    <div style="background-color:#064e3b; border-top:4px solid #10b981; padding:18px; border-radius:10px; text-align:center; color:#ffffff; min-height:140px;">
                        <span style="font-size:0.78rem; color:#a7f3d0; text-transform:uppercase; font-weight:700;">📈 Resgatado WPP / Mês</span>
                        <h3 style="color:#ffffff; font-weight:800; margin:8px 0; font-size:1.4rem;">R$ {val_acum_resg}</h3>
                        <span style="font-size:0.75rem; color:#4ade80;">14 Vendas Aprovadas pós WPP</span>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                with c_a3:
                    ...
                    <div style="background-color:#065f46; border-top:4px solid #34d399; padding:18px; border-radius:10px; text-align:center; color:#ffffff; min-height:140px;">
                        ...
                        <h3 style="color:#ffffff; font-weight:800; margin:8px 0; font-size:1.4rem;">R$ {val_acum_lucro}</h3>
                        <span style="font-size:0.75rem; color:#34d399;">Resgate WPP - Custo Acumulado</span>
                    </div>
                    """, unsafe_allow_html=True)
