    ...
    if menu_selecionado in ['📥 Captação | 📊 Visão Principal de Cadastros', '📊 Visão Principal de Cadastros', '📊 Visão Principal', 'Visão Principal']:
        ...
        try:
            ...
            with tab_p1:
                ...
                st.markdown("<h4 style='color:#ffffff; font-weight:700; margin:24px 0 12px 0;'>📝 Sugestões de Copys Oficiais por Modalidade & Recuperação de Vendas</h4>", unsafe_allow_html=True)
                
                exp_c1, exp_c2, exp_c3, exp_c4, exp_c5 = st.tabs([
                    "📄 Boleto Gerado",
                    "⚡ PIX Gerado",
                    "💳 Cartão Recusado",
                    "🛒 Carrinho Abandonado",
                ...
                with exp_c1:
                    st.markdown("""
                    <div style="background-color:#0f172a; border-left:4px solid #f59e0b; padding:15px; border-radius:8px; margin-bottom:12px;">
                        <b style="color:#f59e0b; font-size:0.9rem;">TAG: boletogerado_cmde</b><br>
                        <span style="color:#cbd5e1; font-size:0.83rem;"><b>Objetivo:</b> Confirmar a emissão do boleto, reforçar a linha digitável e incentivar a migração para PIX/Cartão para acesso imediato.</span>
                    </div>
                    """, unsafe_allow_html=True)
                    st.code("""Olá, {{nome}}! Seu boleto do Curso Mundo dos Elétricos foi gerado com sucesso. 📄

Para garantir sua vaga, basta efetuar o pagamento. Use a linha digitável abaixo no app do seu banco:
{{linha_digitavel}}

⚠️ Atenção: o boleto pode levar até 2 dias úteis para compensar. Assim que o pagamento for confirmado, seu acesso é liberado automaticamente.

⚡ Dica de Ouro (Acesso Instantâneo): Quer começar a assistir hoje mesmo? Você também pode pagar via PIX copia e cola ou cartão pelo link:
{{link}}

Qualquer dúvida, fale com nossa especialista Gabriela: {{especialista}}""", language="markdown")

                with exp_c2:
                    st.markdown("""
                    <div style="background-color:#0f172a; border-left:4px solid #10b981; padding:15px; border-radius:8px; margin-bottom:12px;">
                        <b style="color:#10b981; font-size:0.9rem;">TAG: pixgerado_cmde</b><br>
                        <span style="color:#cbd5e1; font-size:0.83rem;"><b>Objetivo:</b> Entregar o código PIX Copia e Cola e criar urgência amigável antes do vencimento do código.</span>
                    </div>
                    """, unsafe_allow_html=True)
                    st.code("""Olá, {{nome}}! Seu PIX do Curso Mundo dos Elétricos já está gerado. ⚡

O PIX é instantâneo: assim que o pagamento for identificado, seu acesso é liberado na hora.

Copie o código PIX abaixo e pague no app do seu banco (opção PIX Copia e Cola):
{{codigo_pix}}

🚀 Recomendamos concluir o quanto antes para não perder sua vaga (o código tem tempo de validade).

Já paguei e não recebi? Me chame aqui ou fale com nossa especialista: {{especialista}}""", language="markdown")

                with exp_c3:
                    st.markdown("""
                    <div style="background-color:#0f172a; border-left:4px solid #ef4444; padding:15px; border-radius:8px; margin-bottom:12px;">
                        <b style="color:#ef4444; font-size:0.9rem;">TAG: erro_cmde</b><br>
                        <span style="color:#cbd5e1; font-size:0.83rem;"><b>Objetivo:</b> Tranquilizar o lead (que costuma achar que é um problema pessoal) e oferecer opções flexíveis (2 cartões, PIX + Cartão, etc.).</span>
                    </div>
                    """, unsafe_allow_html=True)
                    st.code("""Olá, {{nome}}! Tudo bem?
Notamos que o pagamento do Curso Mundo dos Elétricos não foi aprovado pela operadora do seu cartão. Isso é mais comum do que parece (geralmente limite diário ou trava do banco) e resolve rápido.

👉 Você pode tentar novamente por aqui, com o mesmo cartão, outro cartão ou modalidades flexíveis (PIX ou Boleto):
{{link}}

⚡ Se precisar de ajuda para concluir ou suporte no parcelamento, fale com a equipe: {{suporte}}""", language="markdown")

                with exp_c4:
                    st.markdown("""
                    <div style="background-color:#0f172a; border-left:4px solid #38bdf8; padding:15px; border-radius:8px; margin-bottom:12px;">
                        <b style="color:#38bdf8; font-size:0.9rem;">TAG: carrinho_cmde / Copys de Recuperação (A, B, C, D e Resgate)</b><br>
                        <span style="color:#cbd5e1; font-size:0.83rem;"><b>Objetivo:</b> Resgatar quem abandonou o checkout combatendo a insegurança técnica ("será que eu consigo?").</span>
                    </div>
                    """, unsafe_allow_html=True)
                    st.code("""[COPY DIRETA - FOCO EM SEGURANÇA E MÉTODO DO ZERO]
Fala, {{nome}}! Tafarell aqui. 👊⚡

Vi que você iniciou sua inscrição no Mundo dos Elétricos, mas não chegou a finalizar.
Muita gente nessa faixa de idade me pergunta: "Tafarell, eu não tenho experiência, vou conseguir?"
É exatamente por isso que o curso é 100% passo a passo, prático e simples de acompanhar.

👉 Seu link de retomada está aqui: {{link}}
Se ficou alguma dúvida sobre o pagamento ou conteúdo, me responde por aqui!

---
[COPY COM PROVA SOCIAL / DEPOIMENTO]
Fala, {{nome}}! 👊⚡ Antes de você decidir, quero te mostrar uma coisa:
🎥 {{video_depoimento}}

Esse é o {{nome_aluno}}. Ele começou sem saber nada e hoje atende clientes de elétricos com segurança.
👉 Se fizer sentido para você dar esse passo: {{link}}""", language="markdown")

                with exp_c5:
                    st.markdown("""
                    <div style="background-color:#0f172a; border-left:4px solid #a855f7; padding:15px; border-radius:8px; margin-bottom:12px;">
                        <b style="color:#a855f7; font-size:0.9rem;">🎯 Promessas & Hooks de Copy para CPLs e Tráfego Pago</b><br>
                        <span style="color:#cbd5e1; font-size:0.83rem;"><b>Objetivo:</b> Alinhar anúncios e roteiros de CPL com a demografia real (45-54 anos, leiga/básica).</span>
                    </div>
                    """, unsafe_allow_html=True)
                    st.markdown("""
                    <ul style="color:#e2e8f0; font-size:0.88rem; line-height:1.7;">
                        <li><b>Hook 1 (Eliminando Medo de Idade/Técnico):</b> <i>"Você não precisa de faculdade nem de anos de experiência prévia para dominar a manutenção de bikes e scooters elétricas. Um método simples e visual do zero."</i></li>
                        <li><b>Hook 2 (Nova Fonte de Renda Segura):</b> <i>"A frota elétrica está nas ruas e falta mão de obra qualificada. Descubra como ter uma fonte de renda altamente lucrativa trabalhando no seu próprio ritmo."</i></li>
                        <li><b>Hook 3 (Sem Jargões Desnecessários):</b> <i>"Nada de teorias difíceis ou esquemas elétricos complexos. Aqui você aprende na prática o teste e a solução rápida para os principais defeitos."</i></li>
                    </ul>
                    """, unsafe_allow_html=True)
